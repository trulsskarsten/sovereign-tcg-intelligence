 ...
 */
export function generateStrategyInsights(inventory: any[]): StrategyInsight[] {
  const insights: StrategyInsight[] = [];

  // Logic: Identify Overstock in Stale Classes
  const staleClassC = inventory.filter(i => i.abcClass === 'C' && i.daysInStock > 90);
  ...
  const movers = inventory.filter(i => i.status === 'Movers' && i.abcClass === 'A');
  if (movers.length > 0) {
    insights.push({
      id: 'margin-a-boost',
      type: 'Opportunity',
